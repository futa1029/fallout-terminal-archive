...
2. **X投稿フォーマットのルール**：
   ...
   - ハッシュタグは先頭にまとめて記載。ルールは以下の通り：
     ...
     - それ以外は `#Fallout4` `#Fallout76` `#FalloutNV` `#FalloutTV` など登場作品に合わせて付ける
   - **「☢️ 翻訳」「📝 校正」などのセクション見出しは不要**。セクションの中身（本文）だけを記載し、見出しは絵文字付き小見出し（「🔬 生物学と生態」等）のみ使用する
   ...
   - **内容の言い換え・要約・追加は禁止**。ロア記事（下書きmd）の文章をそのまま転記し、X投稿フォーマット（改行・絵文字・---）だけを付加すること
   - **セクション構成**：翻訳→校正（概要→生物学と生態→利用と噂 など）→感想 の順で全文掲載
