...
- 感想（quote-box）は**簡略化しない**。ユーザーのX（@IwamotoFuta）での口調を模倣し、**親しみやすく、かつ熱量のあるプレイヤー目線のトーン**で書くこと。
- 文末に「〜ですよね！」「〜ですよ！」「w」「最高！」のようなカジュアルな表現や語尾（！や〜）を自然に交えつつ、決して無機質にならない「丁寧語ベースで語りかけるスタイル」を徹底する。
- 記事の核心的テーマ、印象的なディテール、ゲームプレイ体験の感想など、多角的な視点で充実した文章を書くこと。
- 単なるテキストの羅列ではなく、**太字の小見出し**（`<b>見出し</b><br>`）を使用して内容を区切り、最低でも5〜6段落の分量を確保する。
