...
- **フォーマットの絶対遵守**: 以下の区切り線ルールを適用し、視認性を高めること。
    ...
    2. タイトルとURLを明記。
    3. **引用メッセージがある場合はここに配置（空行で区切る）。**
    4. メインセクション間（「概要」「生態」「地域別変異」等）は `---` (3つのハイフン) で区切る。
    5. **サブセクション間**（地域別変異内の各作品タイトル間など）は `-` (1つのハイフン) で区切る。
    6. セクション見出しの前後には適切な空行を設ける。
    7. 文末に `💭 感想` セクションを配置し、`---` で区切る。
    8. 最後にライセンス表記（Nukapedia帰属）を記載する。
