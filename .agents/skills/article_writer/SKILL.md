...
- **ロケーション記事のノート・ホロテープ・ターミナルエントリの翻訳**：ロケーション記事を作成する際、そのロケーションで発見できるノート・ホロテープ・ターミナルエントリがWikiに記載されている場合は、それら関連ページにもアクセスして全文を取得し、日本語に翻訳すること。翻訳したノート内容は記事本文中に専用セクション（例: `<h2>発見できるノート</h2>`）を設けて掲載し、**1つの記事ページ内で完結するようにする**。ノートの原文テキストは `quote-box` とは別の専用スタイル（`border-left` 付きの引用ブロック等）で表示すること
