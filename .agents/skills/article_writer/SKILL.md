...
- **`float` プロパティは絶対に使用しない**。`.content` 内のレイアウトはグリッドで構成されており、`float` は後続要素を巻き込んでレイアウトを崩壊させる。画像は常に `article-image` クラスの標準レイアウト（`display: block; margin: 30px auto;`）で中央配置にすること。
- テキストの横に画像を回り込ませたい場合でも、`float` ではなくグリッドやフレックスボックスで対処すること。
