...
- **フォーマットは `stealth-boy` スタイルを絶対遵守とする**：
    1. 冒頭に適切なハッシュタグ（`#Fallout_TV` または `#Falloutシリーズ`）を配置。
    2. タイトルとURLを明記。
    3. 記事本文を「概要」と「各セクション（`---`区切り）」で構成。
    4. 文末に `💭 感想` セクションを配置。
    5. 最後にライセンス表記（Nukapedia帰属）を記載。
- **不要なUIの完全排除**: `寄付案内`、`コメント欄UI`、`< BACK`、`いいね数` などのウェブサイト専用文字列を一切含めてはならない。
- **画像のLocal管理**: 記事で使用した全画像を `_X/記事名/images/` にコピーする。X用画像は投稿用に4枚程度に厳選することを推奨するが、基本は全コピーで保持する。**X用のディレクトリはLocalのみで管理し、リポジトリへのPushは行わないこと。**
