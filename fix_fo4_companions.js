...
for (let file of htmlFiles) {
    ...
    // ONLY process files that have `style.css`
    if (!raw.includes('style.css')) {
        continue;
    }
    ...
    let category = "人物";
    let appearance = "Fallout 4"; // default for these companions
    let enTitle = "";
    let jpTitle = "";
    let imgPath = "";
    let infoboxRows = "";
    ...
    // Extract Titles
    let h1Match = raw.match(/<h1>(.*?)<\/h1>\s*<div[^>]*>(.*?)<\/div>/s);
    if(h1Match) {
    ...
    let infoboxRowsArray = [];
    let ibRegex = /<div class="infobox-header">(.*?)<\/div>\s*<div class="infobox-data">(.*?)<\/div>/gis;
    let m;
    while((m = ibRegex.exec(raw)) !== null) {
        infoboxRowsArray.push(`<div class="infobox-row"><span class="infobox-label">${m[1].trim()}</span><span>${m[2].trim()}</span></div>`);
    }
    infoboxRows = infoboxRowsArray.join('\\n            ');

    // Main Content
    let contentMatch = raw.match(/<\/div>\s*<\/div>\s*<p>(.*?)<div class="note-box"/s);
    if (contentMatch) {
        strContent = '<p>' + contentMatch[1].trim();

        // Change <div class="quote-box"> to <div class="character-quote"> to avoid collision with standard impression style at the end if there is any inside content
        // BUT wait, in standard template we have quote-box for impression.
        // Actually, older files used <div class="quote-box"> for character quotes at top!
        // We can just keep it as <div class="character-quote"> inside strContent
        strContent = strContent.replace(/<div class="quote-box">/g, '<div class="character-quote">');
        
        // Remove <!-- CONTENT_START --> and <!-- CONTENT_END -->
        strContent = strContent.replace(/<!-- CONTENT_START -->/g, '');
        strContent = strContent.replace(/<!-- CONTENT_END -->/g, '');
    }

    // Impression Quote Box
    let impressionMatch = raw.match(/<h3[^>]*>【MOHIの所感 \(Impression\)】<\/h3>(.*?)<\/div>\s*<!-- CONTENT_END -->/s);
    if(!impressionMatch) {
       impressionMatch = raw.match(/<h3[^>]*>【MOHIの所感 \(Impression\)】<\/h3>(.*?)<\/div>/s);
    }
    
    if (impressionMatch) {
        quoteStr = impressionMatch[1].trim();
    } else {
        quoteStr = "（準備中）";
    }
    ...
    // Hardcode image placeholders for now, maybe we can fetch them from image folders if they exist
    let possibleImagePath = `images/note_extracted/${slug}/${enTitle}.png`;
    // Fandom Wiki sometimes has different names. Let's just use empty string to omit image or use placeholder.
    imgPath = "";
...
console.log("Fixed " + fixedCount + " older companion files.");
