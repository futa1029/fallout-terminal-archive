...
<head>
    ...
    <meta property="og:image" content="https://www.fallout-jp.com/${imgPath}">
...
${quoteStr}
            ...
            <div style="margin-top: 30px; border-top: 1px dashed var(--accent-color); padding-top: 20px; font-size: 0.85em; color: #888;">
                <p name="copyright-default">This article was created by translating and editing <a href="https://fallout.fandom.com/wiki/${enTitle.replace(/ /g, '_')}" target="_blank" rel="noopener">${enTitle}</a> from <a href="https://fallout.fandom.com/" target="_blank" rel="noopener">Nukapedia: The Fallout Wiki</a>.<br>Licensed under the <a href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noopener">Creative Commons Attribution-Share Alike License (CC BY-SA 3.0)</a>.</p>
...
for (let file of files) {
    ...
    // If the file does not have the custom class structures, we need to extract from it carefully.
    // Try to get category
    ...
    // Extract Titles (we usually had: <h1>Japanese<br><span...>English</span></h1>)
    // Wait, in the files I generated, I did: <h1>Japanese<br><span...>English</span></h1>
    let h1Match = raw.match(/<h1>(.*?)<br>\s*<span[^>]*>(.*?)<\/span><\/h1>/s);
    if(h1Match) {
        jpTitle = h1Match[1].trim();
        enTitle = h1Match[2].trim();
    } else {
        // look for old title structure just in case
    ...
    // Extract infobox (My injected version: <div class="infobox">...</div>)
    let infoMatch = raw.match(/<div class="infobox">(.*?)<\/div>\s*<(?:p|h2|div)/s);
    if(infoMatch) {
    ...
    } else if(slug === 'nuka-girl') {
        imgPath = "images/note_extracted/nuka-girl/NukaGirlSpaceSuit.png";
        infoboxRows = `<div class="infobox-row"><span class="infobox-label">分類</span><span>キャラクター</span></div>`;
    }

    // Extract Impression/Quote
    // <div class="quote-box"><b>Impression</b><br><br>(準備中)<br>...</div>
    let quoteMatch = raw.match(/<div class="quote-box">\s*<b>[^<]*<\/b><br><br>(.*?)<\/div>/s);
    if(quoteMatch) {
        quoteStr = quoteMatch[1].trim();
    } else {
        // Fallback or skip if not found
        let quoteMatch2 = raw.match(/<div class="character-quote">(.*?)<\/div>/s);
        if(quoteMatch2) quoteStr = quoteMatch2[1].trim();
    }

    // Extract Main Content
    // Start from after </h1> or end of infobox, up to <div class="quote-box">
    let contentRegex = /<\/div>\s*(<(?:p|h2)>.*?)<div class="quote-box">/s;
    let mainMatch = raw.match(contentRegex);
    if(mainMatch) {
        strContent = mainMatch[1];
    } else if (slug === 'nuka-girl') {
        let nukaMain = raw.match(/<\/div>\s*(<p><b>ヌカ・ガール.*?<\/h3>)/s);
        if(nukaMain) strContent = nukaMain[1];
    }
