...
const htmlFiles = fs.readdirSync(dirPath).filter(f => f.endsWith('.html') && !f.includes('lore.html') && !f.includes('changelog.html') && !f.includes('admin.html'));

function getTemplate(enTitle, jpTitle, slug, category, appearance, imgPath, infoboxRows, contentStr, quoteStr) {
    if (!imgPath && typeof infoboxRows !== 'string') {
        infoboxRows = "";
    }
    return `<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <title>${enTitle} | Overseer Mohi's Terminal</title>
    <link rel="canonical" href="https://www.fallout-jp.com/${slug}.html">
    <meta property="og:type" content="article">
    <meta property="og:site_name" content="Overseer Mohi's Terminal">
    <meta property="og:locale" content="ja_JP">
    <meta property="og:title" content="${enTitle} | Overseer Mohi's Terminal">
    <meta property="og:description" content="${jpTitle}。Falloutのロア記事。">
    <meta property="og:image" content="https://www.fallout-jp.com/${imgPath || 'images/placeholder.jpg'}">
    <meta property="og:url" content="https://www.fallout-jp.com/${slug}.html">
    ...
    <style>
        ...
        .gallery-section { margin-top: 50px; } .gallery-section h2 { margin-bottom: 20px; }
        ...
        @media (max-width: 768px) { .container { grid-template-columns: 1fr; padding: 10px; gap: 20px; } .infobox { grid-row: 1; width: 100%; box-sizing: border-box; position: static; } .content { padding: 15px; } h1 { font-size: 1.6em; } .gallery-grid { grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); } }
        ...
        .terminal-entry { border-left: 4px solid #00ff00; margin: 20px 0; background: rgba(0, 255, 0, 0.05); padding: 15px 15px 15px 20px; border-radius: 0 5px 5px 0; line-height: 1.7; font-family: 'Share Tech Mono', monospace; font-size: 0.9em; color: #00ff00; }
        .terminal-entry b { color: #00ff00; font-size: 1.05em; }
        .holotape-box { border-left: 4px solid #e89b3e; margin: 30px 0; background: rgba(232,155,62,0.08); padding: 15px 15px 15px 20px; border-radius: 0 5px 5px 0; line-height: 1.7; }
        .holotape-box b { color: #e89b3e; font-size: 1.05em; }
    </style>
</head>
<body data-article-category="${category}" data-article-appearance="${appearance}">
    <div class="container">
        <aside class="infobox">
            <h3 style="margin-top:0; text-align:center;">${enTitle}</h3>
            ${imgPath ? `<img src="${imgPath}" alt="${jpTitle}">` : ''}
            ${infoboxRows}
        </aside>
        ...
        <main class="content">
            <div class="action-header">
                ...
                <button class="like-button" data-article-id="note_${slug.replace(/-/g, '_')}" onclick="toggleLike(this)">
                    <span class="heart">♡</span> <span class="like-count">0</span>
                </button>
            </div>

            <h1>${enTitle}<br><span style="font-size: 0.6em; color: #888; font-family: 'Noto Sans JP', sans-serif; font-weight: normal;">${jpTitle}</span></h1>
...
${quoteStr}
            ...
            <div style="margin-top: 30px; border-top: 1px dashed var(--accent-color); padding-top: 20px; font-size: 0.85em; color: #888;">
                <p name="copyright-default">This article was created by translating and editing <a href="https://fallout.fandom.com/wiki/${enTitle.replace(/ /g, '_').replace(/'/g, '%27')}" target="_blank" rel="noopener">${enTitle}</a> from <a href="https://fallout.fandom.com/" target="_blank" rel="noopener">Nukapedia: The Fallout Wiki</a>.<br>Licensed under the <a href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noopener">Creative Commons Attribution-Share Alike License (CC BY-SA 3.0)</a>.</p>
    ...
    <script>
        ...
        const _commentArticleId = 'note_${slug.replace(/-/g, '_')}'; const _commentArticleName = '${enTitle.replace(/'/g, "\\'")}'; const _commentArticleUrl = '${slug}.html'; const ADMIN_TOKEN_KEY = 'fallout_admin_token'; const ADMIN_PASSWORD = 'tq7jtq7j'; const RATE_LIMIT_KEY = 'comment_last_posted'; const RATE_LIMIT_SEC = 60; let _isAdminMode = false;
...
for (let file of htmlFiles) {
    ...
    // ONLY process files that have `index.css` OR `class="scanlines"`
    if (!raw.includes('index.css') && !raw.includes('class="scanlines"')) {
        continue;
    }
    ...
    let category = "人物";
    let appearance = "Fallout TV"; // default
    let enTitle = "";
    let jpTitle = "";
    let imgPath = "";
    let infoboxRows = "";
    ...
    let catMatch = raw.match(/data-article-category="([^"]+)"/);
    if(catMatch) category = catMatch[1];

    let appMatch = raw.match(/data-article-appearance="([^"]+)"/);
    if(appMatch) appearance = appMatch[1];

    let h1Match = raw.match(/<h1>(.*?)<br>\s*<span[^>]*>(.*?)<\/span><\/h1>/s);
    if(h1Match) {
        jpTitle = h1Match[1].replace(/<[^>]*>/g, '').trim();
        enTitle = h1Match[2].replace(/<[^>]*>/g, '').trim();
    } else {
        let titleMatch = raw.match(/<title>([^<(]+?)(?:\s*\(([^)]+)\))?/);
        if(titleMatch) {
            jpTitle = titleMatch[1].trim();
            if(titleMatch[2]) enTitle = titleMatch[2].trim();
            else enTitle = jpTitle;
        }
    }

    let infoMatch = raw.match(/<div class="infobox">(.*?)<\/div>\s*<(?:p|h2|div|img class="article-image")/s);
    if(infoMatch) {
        let infoboxRaw = infoMatch[1];
        let imgMatch = infoboxRaw.match(/<img[^>]*src="([^"]+)"/);
        if(imgMatch) imgPath = imgMatch[1];
        
        let rows = [];
        let rowRegex = /<div class="info-label">([^<]+)<\/div><div class="info-value">([^<]+)<\/div>/g;
        let m;
        while((m = rowRegex.exec(infoboxRaw)) !== null) {
            rows.push(`<div class="infobox-row"><span class="infobox-label">${m[1]}</span><span>${m[2]}</span></div>`);
        }
        infoboxRows = rows.join('\\n            ');
    }

    let quoteMatch = raw.match(/<div class="quote-box">\s*<b>[^<]*<\/b><br><br>(.*?)<\/div>/s);
    if(quoteMatch) {
        quoteStr = quoteMatch[1].trim();
    } else if (raw.includes('class="character-quote"')) {
        let qsMatch = raw.match(/<div class="character-quote">(.*?)<\/div>/s);
        if (qsMatch) quoteStr = qsMatch[1].trim();
    }

    // Extract main content after </h1>
    // In our broken markup: </h1> ... <p style="color:#666...
    // Or in some others, from </h1> down to "</main>"
    let mainRegex = /<\/h1>\s*(?:<div class="infobox">.*?<\/div>\s*)?(.*?)<p style="color:#666/s;
    let mainMatch = raw.match(mainRegex);
    if(mainMatch) {
        strContent = mainMatch[1];
        // Ensure we remove the trailing quote-box if it got sucked in
        strContent = strContent.replace(/<div class="quote-box">[\s\S]*$/, '').trim();
    } else {
         // fallback
         let mRegex2 = /<\/h1>\s*((?:<div class="infobox">.*?<\/div>\s*)?.*?)(?:<\/main>|<p name="copyright-default">|<div class="quote-box">)/s;
         let m2 = raw.match(mRegex2);
         if (m2) {
             strContent = m2[1].replace(/<div class="infobox">.*?<\/div>\s*/s, '').trim();
         }
    }

    if (!jpTitle) jpTitle = slug;
    if (!enTitle) enTitle = slug;
    
    if(!strContent) {
        console.log("Failed to match content for", file);
        continue;
    }

    let finalHtml = getTemplate(enTitle, jpTitle, slug, category, appearance, imgPath, infoboxRows, strContent, quoteStr);
