...
const main = async () => {
    const title = "Axel (Milepost Zero)";
    console.log(`Fetching: ${title}`);
    const content = await fetchWikitext(title);
    
    fs.writeFileSync('f:/Fallout/_wikitext_batch30.json', JSON.stringify({ axel: content }, null, 2));
    console.log('Batch 30 wikitext fetched.');
};
