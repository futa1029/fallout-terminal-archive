...
const processFile = (filename, pId, enName, jpName) => {
    ...
    // Special fix for seneca-rocks
    if (!copyrightFixed && filename === 'seneca-rocks.html') {
        const brokenDiv = document.querySelector('div[style*="font-size: 0.85em"]');
        if (brokenDiv && brokenDiv.innerHTML.includes('undefined')) {
            let encURI = encodeURIComponent(enName).replace(/%20/g, "_");
            brokenDiv.innerHTML = `
                <p name="copyright-default">This article was created by translating and editing <a href="https://fallout.fandom.com/wiki/${encURI}" target="_blank" rel="noopener">${enName}</a> from <a href="https://fallout.fandom.com/" target="_blank" rel="noopener">Nukapedia: The Fallout Wiki</a>.<br>Licensed under the <a href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noopener">Creative Commons Attribution-Share Alike License (CC BY-SA 3.0)</a>.</p>
...
processFile('seneca-rocks.html', 'note_seneca_rocks', 'Seneca Rocks', 'セネカ・ロックス');
processFile('vendor-bot-phoenix.html', 'note_vendor_bot_phoenix', 'Vendor bot Phoenix', 'ベンダーボット・フェニックス');
processFile('hannah-wastelanders.html', 'note_hannah_wastelanders', 'Hannah', 'ハンナ');
processFile('lotus.html', 'note_lotus', 'Lotus', 'ロータス');
