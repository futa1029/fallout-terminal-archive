...
const batch2DataPath = 'f:/Fallout/_drafts/medium_batch2_eng.json';
const listPath = 'f:/Fallout/_drafts/tv_locations_list.json';
const outputPath = 'f:/Fallout/_drafts/medium_batch3_eng.json';
...
let batch2Data = {};
try { batch2Data = JSON.parse(fs.readFileSync(batch2DataPath, 'utf8')); } catch (e) {}
...
const batch2Names = Object.keys(batch2Data).map(k => k.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''));
const massiveNames = [
    "Vault 4",
    "Primm",
    "Mojave Wasteland",
...
const completedSlugs = new Set([...smallNames, ...batch1Names, ...batch2Names, ...massiveNames]);
...
const batch = remaining; // all remaining
...
console.log('--- Medium Batch 3 (Final ' + batch.length + ' locations) ---');
