...
const filesToFix = [...brokenData.mostlyEnglish, ...brokenData.fullEnglish];
...
const getStandardScript = (id, name, url) => `
        ...
        const _commentArticleId = '${id}'; const _commentArticleName = '${name.replace(/'/g, "\\'")}'; const _commentArticleUrl = '${url}'; const ADMIN_TOKEN_KEY = 'fallout_admin_token'; const ADMIN_PASSWORD = 'tq7jtq7j'; const RATE_LIMIT_KEY = 'comment_last_posted'; const RATE_LIMIT_SEC = 60; let _isAdminMode = false;
...
for (const filename of filesToFix) {
    ...
    const articleId = likeBtn ? likeBtn.getAttribute('data-article-id') : null;
    let articleName = '';
    const h1 = document.querySelector('h1');
    if (h1) {
        articleName = h1.textContent.split('\n')[0].trim();
        // Remove japanese span if it got mixed in
        if (h1.childNodes[0] && h1.childNodes[0].nodeType === 3) {
            articleName = h1.childNodes[0].textContent.trim();
        }
    }
    ...
    // 2. Fix copyright if totally missing or broken Fandom link
    if (!html.includes('This article was created by translating') && !html.includes('name="copyright-default"')) {
        if (filename === 'appalachian-radio.html') {
            const container = document.querySelector('main.content');
            const copyrightDiv = document.createElement('div');
            copyrightDiv.style.cssText = "margin-top: 30px; border-top: 1px dashed var(--accent-color); padding-top: 20px; font-size: 0.85em; color: #888;";
            copyrightDiv.innerHTML = `
                <p name="copyright-default">This article was created by researching <a href="https://fallout.fandom.com/wiki/Appalachia_Radio" target="_blank" rel="noopener">Appalachia Radio</a> from <a href="https://fallout.fandom.com/" target="_blank" rel="noopener">Nukapedia: The Fallout Wiki</a>.<br>Licensed under the <a href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noopener">Creative Commons Attribution-Share Alike License (CC BY-SA 3.0)</a>.</p>
                <p style="margin-top: 15px;">コミュニティ維持のため、<a href="https://mohi3.fanbox.cc/" target="_blank" rel="noopener" style="color: var(--accent-color);">寄付を受け付けております</a>。</p>
            `;
            const commentSec = document.querySelector('.comments-section');
            if (commentSec) {
                container.insertBefore(copyrightDiv, commentSec);
            } else {
                container.appendChild(copyrightDiv);
            }
            isModified = true;
        } else {
            console.log("WAIT, unhandled missing copyright:", filename);
        }
    } else {
        // Find existing copyright block and wrap it in the proper standard if it doesn't have name="copyright-default"
        const pElements = document.querySelectorAll('p');
        for (let p of pElements) {
            if (p.textContent.includes('This article was created by translating') && !p.hasAttribute('name')) {
                // If the block is unstandardized
                const aTags = p.querySelectorAll('a');
                if (aTags.length >= 2) {
                    const fandomLink = aTags[0].href;
                    const name = aTags[0].textContent;
                    p.outerHTML = `<p name="copyright-default">This article was created by translating and editing <a href="${fandomLink}" target="_blank" rel="noopener">${name}</a> from <a href="https://fallout.fandom.com/" target="_blank" rel="noopener">Nukapedia: The Fallout Wiki</a>.<br>Licensed under the <a href="https://creativecommons.org/licenses/by-sa/3.0/" target="_blank" rel="noopener">Creative Commons Attribution-Share Alike License (CC BY-SA 3.0)</a>.</p>`;
                    isModified = true;
                }
            }
        }
    ...
    if (!html.includes('id="hp_field"')) {
        let commentSec = document.querySelector('.comments-section');
        const stdCommentStr = `
        ...
        if (commentSec) {
            commentSec.outerHTML = stdCommentStr;
        } else {
            const main = document.querySelector('main.content');
            if (main) main.insertAdjacentHTML('beforeend', stdCommentStr);
        }
        isModified = true;
        ...
        if (articleId) {
            ...
            newScript.textContent = getStandardScript(articleId, articleName, filename);
            const body = document.querySelector('body');
            const commonScript = document.querySelector('script[src*="article-common.js"]');
            if (commonScript && body.contains(commonScript)) {
                body.insertBefore(newScript, commonScript);
            } else {
                body.appendChild(newScript);
                let articleCommon = document.createElement('script');
                articleCommon.src = "article-common.js";
                articleCommon.defer = true;
                body.appendChild(articleCommon);
            }
        }
    }
    ...
    if (!html.includes('quote-box') && filename !== 'appalachian-radio.html') {
         // I'll log if quote box is missing, we need to manually write it if so
         console.log("Missing quote-box:", filename);
    }
...
console.log("Total files standardized in this run:", fixCount);
