...
const fixFile = (filepath, id, name, url) => {
    let ht = fs.readFileSync(filepath, 'utf8');
    const dom = new JSDOM(ht);
    const document = dom.window.document;
    
    // remove existing scripts except article-common
    const scripts = document.querySelectorAll('script');
    for (let s of scripts) {
        if (!s.src || !s.src.includes('article-common.js')) {
            if (s.src && s.src.includes('supabase')) {
                // keep the external one
            } else {
                s.remove();
            }
        }
    ...
    newScript.textContent = getStandardScript(id, name, url);
    ...
    console.log(name + ' script fixed.');
};

fixFile('f:/Fallout/xander-brown.html', 'note_xander_brown', 'Xander Brown', 'xander-brown.html');
fixFile('f:/Fallout/windy-park.html', 'note_windy_park', 'Windy Park', 'windy-park.html');
